...
describe PawnsController do
  ...
  describe "when I successfully POST 'create" do 
    ...
    def do_create
      post :create, :pawn =>{:title=>"pawn", :description => "this is a pawn"}
    end
    
    it "should create the pawn" do
      Pawn.should_receive(:new).with("title"=>"pawn", "description" => "this is a pawn").and_return(@pawn)
      do_create
    end
  ...
  describe "when I PUT update" do
    ...
    def do_update
      put 'update', :id => @pawn.id, :pawn => {:title=>"pawn", :description =>"pawn"}
    end
